...
namespace FitTrackPro.Data
{
    public static class DataSeeder
    {
        public static async Task InitializeAsync(ApplicationDbContext context)
        {
            ...
            // Seed exercises first (required by workouts)
            SeedExercises(context);

            // Seed workouts (depends on exercises)
            WorkoutDataSeeder.Initialize(context);

            // Seed body metrics (independent)
            BodyMetricsDataSeeder.Initialize(context);

            // Seed recipes (required by meal plans)
            await RecipeDataSeeder.seedRecipesAsync(context);

            // Seed meal plans (depends on recipes)
            await MealPlanDataSeeder.SeedMealPlansAsync(context);
        }
